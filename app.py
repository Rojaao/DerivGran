...
async def bot_loop(token, stake, threshold, take_profit, stop_loss, multiplicador, estrategia):
    try:
        ...
        if auth.get("error"):
            ...
            return
        st.session_state.logs.append("✅ Autenticado com sucesso")
        ...
        while st.session_state.bot_running:
            if total_profit >= take_profit:
                st.session_state.logs.append(f"🏁 Meta de lucro alcançada: +${total_profit:.2f}")
                break
            if total_profit <= -abs(stop_loss):
                st.session_state.logs.append(f"🛑 Stop loss alcançado: -${abs(total_profit):.2f}")
                break
            ...
            if "tick" in msg and not contract_active and not waiting_buy:
                ...
                if len(digits) == 8:
                    ...
                    all_lt4 = all(d < 4 for d in digits)
                    st.session_state.logs.append(f"📊 Análise: c4={c4}, all<4={all_lt4}")

                    if (estrategia == "Padrão" and c4 >= threshold) or \
                       (estrategia == "0matador" and all_lt4):
                ...
                if c.get("contract_id") == contract_id:
                    ...
                    if status == "won":
                        ...
                        if loss_streak >= 2:
                            ...
                            st.session_state.logs.append(f"🔁 Multiplicador ativado: novo stake = R${current_stake:.2f}")
                    contract_active = False
        await ws.close()
    except Exception as e:
        st.session_state.logs.append(f"❌ Erro: {str(e)}")
        st.session_state.bot_running = False
...
def stop_bot():
    st.session_state.bot_running = False
    st.session_state.logs.append("⛔ Robô parado manualmente.")
